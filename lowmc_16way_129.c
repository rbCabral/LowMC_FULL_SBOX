/*! @file lowmc_16way_129.c
 *  @brief 16-way optimized implementation of the LowMC-129-129-4   
...
#define loadLinha(i)            resp =  _mm256_setzero_si256();\
                                for(j=0;j<9;j++){\
                                    temp = _mm256_loadu_si256((__m256i*)matrix+(i*9+j));\
...
void stateTransFULL(uint8_t in[][17], uint8_t out[][16]){
out[0][0] = in[0][0];
out[16][0] = in[0][1];
out[14][1] = in[0][2];
out[12][2] = in[0][3];
    ...
    const uint8_t* matrix)
{
    ...
    while(1){
        ...
        if(c < 3){
            ...
            /*linha LIXO*/ /*leio p*/
            aux[14] = _mm256_unpacklo_epi8(a,_mm256_setzero_si256());/*p14o14p12o12p10o10p8o8p6o6p4o4p2o2p0o0*/
            aux[15] = _mm256_unpackhi_epi8(a,_mm256_setzero_si256());/*p15o15p13o13p11o11p9o9p7o7p5o5p3o3p1o1*/
        }else{
            i+=9;
            aux[14] = _mm256_setzero_si256();
            aux[15] = _mm256_setzero_si256();
        }
...
void LowMCEnc(uint8_t in[][16], uint8_t out[][16], uint8_t key[][16])
{
    ...
    matrix_mulBitslice(rRoundKey, r, KMatrixBitslice(0, 129));      
   ...
   for (uint32_t r = 1; r <= ROUNDS; r++) {
        ...
        for(i = 0;i<9;i++){
            r2 = _mm256_loadu_si256((__m256i*)ConstantBitslice(r-1,129) + (i));
...
void LowMCEnc2(uint8_t in_t[][17], uint8_t out_t[][17], uint8_t key_t[][17])
{
   ...
   for (uint32_t r = 1; r <= ROUNDS; r++) {
    ...
    stateTrans2Back(out,out_t);
    stateTransBack(&out_t[0] , &out_t[0]);
    stateTransBack(&out_t[8] , &out_t[8]);
    /*--------------------------------*/
...
int main(){
  ...
  uint8_t ciphertext_expected[16][17] = {\
    ...
    uint8_t plaintextAux2[18][16] = {0};
    uint8_t aux[16][17] = {0};
    uint8_t keyAux2[18][16] = {0};
    uint8_t out[18][17] = {0};
    uint8_t outAux[18][16] = {0};

    stateTrans(&plaintext[0],&aux[0]);
    stateTrans(&plaintext[8],&aux[8]);
    stateTransFULL(aux,plaintextAux2);

    stateTrans(&key[0],&aux[0]);
    stateTrans(&key[8],&aux[8]);
    stateTransFULL(aux,keyAux2);

    LowMCEnc(plaintextAux2, outAux, keyAux2);

    stateTrans2Back(outAux,out);
    stateTransBack(&out[0] , &out[0]);
    stateTransBack(&out[8] , &out[8]);
